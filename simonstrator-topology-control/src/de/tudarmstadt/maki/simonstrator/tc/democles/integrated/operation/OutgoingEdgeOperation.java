...
public class OutgoingEdgeOperation extends NativeOperation {
	...
	public InternalDataFrameProvider getDataFrame(RemappedDataFrame frame, Adornment adornment) {
		if (adornment.get(0) == Adornment.BOUND && adornment.get(1) == Adornment.BOUND && adornment.get(2) == Adornment.BOUND) {
			...
			if(graph.getOutgoingEdges(node.getId()).contains(edge)) {
				return frame;
			}
			...
			final Set<IEdge> edges = graph.getOutgoingEdges(node.getId());
			...
			final INode node = graph.getNode(edge.fromId());
			if(node != null){
