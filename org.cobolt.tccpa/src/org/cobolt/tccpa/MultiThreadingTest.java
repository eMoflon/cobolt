...
/**
 * ?? Is reading concurrently from an EMF model safe? I guess not.
 * ?? Wouldn't it be better to use a database (e.g., NeoEMF)?
 *
 * Basic idea:
 * * Establish a global lock for the entire model
 *    * This is different from the lock on the model level!
 * * Question 1: Where to place lock and unlock pairs?
 *    * We cannot simply lock before and after each 'pattern' invocation.
 ...
 * }
 *
 * * Question 2: How to insert the locks?
 *    * Modify code generator (would be parallel to what we did until now)
 *
 * * Question 3: How to realize local views?
 *    * Create separate Resources for each process
 *    * Synchronization
 *       * Target platform: No problems with concurrency. Just use synchronized lists. Allows to use network simulator (<- is this sensible? CPU speed is vastly different from simulation speed!)
 *       * Using GT: Allows static analysis, stochastic GT, ... Fits the spirit of MDE better.
 *       * Holistic approach: hide behind EOperation: Pure spec.: use GT; Simulation: use simulator; Testbed: use messaging API of sensor OS
