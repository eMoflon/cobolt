...
public class TopologyModelTestUtils {
	...
	public static void assertState(final Topology topology, final EdgeState state, final boolean checkSymmetry,
	...
	/**
	 * Asserts that the given edge is **not** in state
	 * {@link EdgeState#UNCLASSIFIED}
	 */
	...
	/**
	 * Asserts that all edges in the given topology that have one of the edgeIds are
	 * in state {@link EdgeState#ACTIVE}
	 */
	...
	/**
	 * Asserts that all edges in the given topology that have one of the edgeIds are
	 * in state {@link EdgeState#INACTIVE}
	 */
	...
	/**
	 * Asserts that all edges in the given topology that have one of the edgeIds are
	 * in state {@link EdgeState#UNCLASSIFIED}
	 */
	...
	/**
	 * Asserts that the graph contains for each edge its reverse edge and that the
	 * state of forward and reverse edge are the same.
	 */
	 ...
	 */
	public static void assertNodeAndEdgeCount(final Topology topology, final int expectedNodeCount,
	...
	public static void assertEdgeDistance(final Topology topology, final String id, final double expectedDistance) {
		Assert.assertEquals("Distance mismatch of " + id + ".", expectedDistance,
				topology.getEdgeById(id).getDistance(), TopologyControlTestHelper.EPS_0);
	}
	...
	public static void assertEdgeWeight(final Topology topology, final String id, final double expectedWeight) {
		Assert.assertEquals("Weight mismatch of " + id + ".", expectedWeight, topology.getEdgeById(id).getWeight(),
				TopologyControlTestHelper.EPS_0);
	}
