...
public class NodeTest
{
   ...
   public void testBuildLocalView_Scalability() throws Exception
   {
      ...
      for (int k = 0; k < 10; ++k)
      {
         ...
         System.out.println(String.format("#k=%d, localViewCount=%d, time=%.2fms", k, edgeCountInLocalView, watch.getNanoTime() / 1e6));
      }
   }

   private static double getAverageNodeDegree(final Topology topology)
   {
      return topology.getNodes().stream().map(node -> node.getDegree()).reduce((a, b) -> a + b).get() / ((double)topology.getNodeCount());
   }
