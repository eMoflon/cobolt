...
public class TotalSentMessageSizeMetric extends AbstractMetric<MetricValue<?>> {

	public TotalSentMessageSizeMetric() {
		super("TotalSentMessageSize", MetricUnit.DATA);
	}

	@Override
	public void initialize(final List<Host> hosts) {
		hosts.forEach(host -> addHost(host, new SimpleNumericMetricValue<Integer>(
				TopologyControlEvaluationApplication_ImplBase.find(host).getTotalMessageSentSize())));
	}
