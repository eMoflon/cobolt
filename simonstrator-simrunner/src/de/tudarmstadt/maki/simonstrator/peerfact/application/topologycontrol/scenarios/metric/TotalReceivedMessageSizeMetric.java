...
public class TotalReceivedMessageSizeMetric extends AbstractMetric<MetricValue<?>> {

	public TotalReceivedMessageSizeMetric() {
		super("TotalReceivedMessageSize", MetricUnit.DATA);
	}

	@Override
	public void initialize(final List<Host> hosts) {
		hosts.forEach(host -> addHost(host, new SimpleNumericMetricValue<Integer>(
				TopologyControlEvaluationApplication_ImplBase.find(host).getTotalMessageReceivedSize())));
	}
