...
public class CheckpointedReceivedMessageSizeMetric extends AbstractMetric<MetricValue<?>> {

	public CheckpointedReceivedMessageSizeMetric() {
		super("ReceivedMessageSizeSinceCheckpoint", MetricUnit.DATA);
	}

	@Override
	public void initialize(final List<Host> hosts) {
		hosts.forEach(host -> addHost(host, new SimpleNumericMetricValue<Integer>(
				TopologyControlEvaluationApplication_ImplBase.find(host).getMessageReceivedSizeSinceCheckpoint())));
	}
