...
public class TotalSentMessageCountMetric extends AbstractMetric<MetricValue<?>> {

	public TotalSentMessageCountMetric() {
		super("TotalSentMessageCount", MetricUnit.NONE);
	}

	@Override
	public void initialize(final List<Host> hosts) {
		hosts.forEach(host -> addHost(host, new SimpleNumericMetricValue<Integer>(
				TopologyControlEvaluationApplication_ImplBase.find(host).getTotalMessageSentCount())));
	}
