...
public class CheckpointedReceivedMessageCountMetric extends AbstractMetric<MetricValue<?>> {

	public CheckpointedReceivedMessageCountMetric() {
		super("ReceivedMessageCountSinceCheckpoint", MetricUnit.NONE);
	}

	@Override
	public void initialize(final List<Host> hosts) {
		hosts.forEach(host -> addHost(host, new SimpleNumericMetricValue<Integer>(
				TopologyControlEvaluationApplication_ImplBase.find(host).getMessageReceivedCountSinceCheckpoint())));
	}
