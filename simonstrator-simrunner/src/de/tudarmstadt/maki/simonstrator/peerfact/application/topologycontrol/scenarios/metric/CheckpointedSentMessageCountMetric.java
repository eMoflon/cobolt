...
public class CheckpointedSentMessageCountMetric extends AbstractMetric<MetricValue<?>> {

	public CheckpointedSentMessageCountMetric() {
		super("SentMessageCountSinceCheckpoint", MetricUnit.NONE);
	}

	@Override
	public void initialize(final List<Host> hosts) {
		hosts.forEach(host -> addHost(host, new SimpleNumericMetricValue<Integer>(
				TopologyControlEvaluationApplication_ImplBase.find(host).getMessageSentCountSinceCheckpoint())));
	}
