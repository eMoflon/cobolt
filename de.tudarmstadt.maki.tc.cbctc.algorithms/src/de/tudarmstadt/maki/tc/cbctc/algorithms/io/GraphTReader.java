...
public class GraphTReader
{
   ...
   private boolean isAutoReverseLinkingActive;
      ...
      try
      {
         ...
         while (readEdgeLines < m)
         {
            ...
            if (isDataLine(line))
            {
               ...
               if (this.isAutoReverseLinkingActive && isEven(readEdgeLines))
               {
    ...
    * @return
    */
   private boolean isMagicComment(String line)
   {
      return line.startsWith("#!");
   }

   private void handleMagicComment(String line)
   {
   ...
   public static Topology readTopology(final String filename) throws FileNotFoundException
   {
      ...
      readTopology(topology, filename);
      return topology;
   }
   ...
   {
      final GraphTReader reader = new GraphTReader();
